...
if (window["_VENDER_AJAX_SIGNWARP_DONE_"] != true || window["_VENDER_AJAX_SIGNWARP_FUN_"] != $.ajax) {
    window["_VENDER_AJAX_SIGNWARP_DONE_"] = true;
    var _tmplibajax_ = $.ajax;
    $.ajax = function(opts) {
        var asignk = "Ajax-Sign";
        var csrf_token="";
        var asignv = "";
        var cookiekvstrs = document.cookie.split("; ");
        for (var i = cookiekvstrs.length - 1; i >= 0 ; i--) {
            var kvarr = cookiekvstrs[i].split("=");
            if (kvarr[0] == "_AJAX_SIGN_KEY_") {
                asignv = kvarr[1];
            }
        }
        if (asignv == "") {
            asignv = (new Date()).getTime() + "asignkeyck";
            document.cookie= "_AJAX_SIGN_KEY_=" +escape(asignv) + ";path=/;";
        }
        var optbeforeSendFun = opts.beforeSend;
        opts.beforeSend = function(request) {
            request.setRequestHeader(asignk, asignv);
            if (typeof optbeforeSendFun == "function") {
                optbeforeSendFun.call(null, request);
            }
        };
        return _tmplibajax_.call(null, opts);
    };
    window["_VENDER_AJAX_SIGNWARP_FUN_"] = $.ajax;
}