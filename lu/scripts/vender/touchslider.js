		...
		slider: function(obj) {
			this.each(function() {
				...
				var settings = {
					...
					"addtrack":obj ? (obj.addtrack ? true : false) : false
				};
				getWidth();
				function getWidth(){
					settings.width= dom.item.width();
					if(window.tab!="tab1"){
						$("#tab1").css({'visibility': 'hidden', 'display': 'block' });
						settings.width= dom.item.width();
						settings.offset= ($(window).width()-dom.item.width())/2;
						$("#tab1").css({'visibility': 'visible', 'display': 'none' });
					}
				}
				var funs = {
					init: function() {
					...
					bindEvent: function() {
						$self.off().on({
							...
							"webkitTransitionEnd": function(e) {
								...
								dom.wrap.css("-webkit-transform", "translate3d(" + -settings.index * settings.width + "px,0,0)");
								if(window.BroadCast&&settings.addtrack){
									BroadCast.notify("pageChange",itemIndex);
								}
							}
						});
						$(window).on({
